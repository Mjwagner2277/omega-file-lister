...
docker run --rm --privileged \
  ...
  sh -lc 'apt-get update >/dev/null && apt-get install -y --no-install-recommends mount util-linux squashfs-tools libarchive-tools xz-utils zstd time >/dev/null && /usr/bin/time -p /lfl -mount-iso /input.iso && wc -l /input_iso_files && cp /input_iso_files /out/input_iso_files'
